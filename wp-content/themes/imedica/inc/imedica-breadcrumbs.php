...
if ( ! function_exists( "imedica_breadcrumb" ) ) {
	...
	function imedica_breadcrumb() {
		/* Set up the arguments for the breadcrumb. */
		$opts = get_option( "rdfa_options" );
		$args = array(
			'prefix'                 => $opts['prefix'],
			'suffix'                 => $opts['suffix'],
			'title'                  => $opts['title'], //__( ' ', 'imedica' ),
			'home_title'             => $opts['home_title'], //__( 'Home', 'imedica' ),
 ...
 * @since 0.4
 */
if ( ! function_exists( 'rdfa_breadcrumb_get_items' ) ) {
	...
	function rdfa_breadcrumb_get_items( $args ) {
		...
		/* Link to front page. */
		if ( ! is_front_page() ) {
			$item[] = '<span typeof="v:Breadcrumb"><a rel="v:url" property="v:title" href="' . home_url( '/' ) . '" class="fa fa-home"><span class="hidden_title">Home</span></a></span>';
		}
		...
		elseif ( is_singular() ) {
			...
			if ( is_category() || is_tag() || is_tax() ) {
				...
				if ( is_day() ) {
					$item['last'] = __( 'Archives for ', 'imedica' ) . get_the_time( 'F j, Y' );
				} elseif ( is_month() ) {
					$item['last'] = __( 'Archives for ', 'imedica' ) . single_month_title( ' ', false );
				} elseif ( is_year() ) {
					$item['last'] = __( 'Archives for ', 'imedica' ) . get_the_time( 'Y' );
				}
			} else if ( is_author() ) {
				$item['last'] = __( 'Archives by: ', 'imedica' ) . get_the_author_meta( 'display_name', $wp_query->post->post_author );
			}
		} /* If viewing search results. */
		else if ( is_search() ) {
			$item['last'] = __( 'Search Results for - ', 'imedica' ) . stripslashes( strip_tags( get_search_query() ) ) . '"';
		} /* If viewing a 404 error page. */
		else if ( is_404() ) {
			$item['last'] = __( 'Page Not Found', 'imedica' );
		}
