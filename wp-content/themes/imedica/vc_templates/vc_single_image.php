...
/* Code for iMedica start */

if( $image != "" && $source !== 'external_link' ) {
	$img_arr = explode("|", $image);

	if( array_key_exists( 1, $img_arr ) ) {
		list( $imageId, $imageURL ) = $img_arr;

		$imageURL = str_replace( 'url^', '', $imageURL );
		$imageId = str_replace( 'id^', '', $imageId );

		$attachment_src = wp_get_attachment_image_src( $imageId, 'full' );

		if ( $attachment_src !== '' ) {
			$attachmeny_img = basename( $attachment_src[0] );
			$shortcode_url = basename( $imageURL );
			if ( $attachmeny_img == $shortcode_url && $imageId !== 'null' ) {
				$image = $imageId;	
			} else {
				$source = 'external_link';
				$custom_src = $imageURL;
				$external_style = $style;
				$external_border_color = $border_color;
				$external_img_size = $img_size;
				// set rectangular
				if ( preg_match( '/_circle_2$/', $external_style ) ) {
					$external_style = preg_replace( '/_circle_2$/', '_circle', $external_style );
				}
			}
		} else {			 
			$source = 'external_link';
...
if ( 'yes' === $add_caption && '' !== $caption ) {
	$html = '
	<figure class="vc_figure" style="width:100%;">
		' . $html . '
